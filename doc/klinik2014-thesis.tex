...
Greatest fixpoints of functors arise as limits of left chains. See for example
theorem 11.3.14 in \cite{Manes1986}.  \begin{IEEEeqnarray*}{c}
\cdots\ \arr\ F^3(1)\ \arr\ F^2(1)\ \arr\ F(1)\ \arr\ 1
\end{IEEEeqnarray*}
Intuitively, the cppos we use as data structure semantics arise by $\omega$-many
iterations of the base functor on the terminal object $1$. The first few
iterations for the functor $FX = 1 + X$, together with the partial order induced
by the separated sum, look as in figure \ref{figFirstIterationsOfF}. In each
iteration, the cppo of the previous step is lifted through $\kappa_1$ to the
upper right, and fresh $\kappa_0(\bot)$ and $\bot$ are introduced.

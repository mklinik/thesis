...
The coalesced sum is not useful (todo: that's just plain wrong!) for categorical fixpoint semantics of
...
Algebraic data types (\emph{ADT}) are a way to allow the programmer to add new
base types besides the built-in ones to the programming language. As feature of
the formal system of a programming language, they need to be incorporated into
syntax and semantics. While the details of the concrete syntax differs ...

it's always sums of products where factors are either constant or recursive -
empty product is allowed, but not empty sum

<ADT> ::= data <ID> = <CONSTRUCTOR> (| <CONSTRUCTOR>)*
...
\matrix (m) [matrix of math nodes,row sep=3em,column sep=4em,minimum width=2em]
  ...
  \path[-stealth]
    (m-1-1) edge node [left] {eval} (m-2-1)
    (m-1-1) edge node [above] {translate} (m-1-2)
    (m-2-1.east|-m-2-2) edge node [below] {translate} (m-2-2)
    (m-1-2) edge node [right] {eval} (m-2-2);
\end{tikzpicture}
\end{center}
\end{figure}

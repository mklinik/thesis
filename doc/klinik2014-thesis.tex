...
algebraic datatype, and study from a computational viewpoint how it relates to
...
Domain theory offers tools to approach answers to such questions. We are
...
more intuitive. We encourage the reader to think in terms of non-decreasing
...
\end{tikzpicture}
\end{center}
The diagram commutes because our category has coproducts, and together with $\kappa_0 = \kappa_1$ we get $\bot_A
...
allowed to map $\bot$ to elements other that $\bot$. This is in fact desired, otherwise the fixpoint
...
\ref{propCCCNoFixpointCoproducts} in this form are due to Huwig
...
constructions are useful for programming language semantics, as will be seen in section
...
briefly look at algebraic datatypes and informally argue how polynomial functors
can be used to give denotational semantics for them.

Algebraic datatypes (\emph{ADT}s) allow the programmer to add new base types
...
call-by-name and call-by-value we would like the one-valued datatype
\begin{verbatim}
data Unit = Unit
\end{verbatim}
...
Consider as another example the natural numbers. We want the datatype
\begin{verbatim}
data Nat = Zero | Succ Nat
\end{verbatim}
to denote the flat naturals in call-by-value languages, and the lazy naturals
for call-by-name. This datatype is recursive, and denotations of recursive
datatypes arise by iterating the appropriate functor on the singleton cppo, see
...
the coinductive natural numbers. In this section we show that any datatype
...
fixpoints to guarantee well-definedness of desired constructs.
...
categorical coproducts is due to Huwig and Poign\'e \cite{Huwig1990}. These
results were known before as folklore theorems, Huwig and Poign\'e coined the

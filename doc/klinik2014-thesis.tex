...
in a given situation. As we are exclusively interested in the countable case,
whenever we consider chains, we mean countable ones. A countable chain $\set{x_n
\mid x \in \bbN}$ is always also a non-decreasing sequence
...
Greatest fixpoints of functors arise as limits of left chains.
\begin{IEEEeqnarray*}{c}
\cdots\ \arr\ F^3(1)\ \arr\ F^2(1)\ \arr\ F(1)\ \arr\ 1
\end{IEEEeqnarray*}
See for example theorem 11.3.14 in \cite{Manes1986}.  Intuitively, the cppos we
use as data structure semantics arise by $\omega$-many iterations of the base
functor on the terminal object $1$. The first few iterations for the functor $FX
= 1 + X$, together with the partial order induced by the separated sum, look as
in figure \ref{figFirstIterationsOfF}. In each iteration, the cppo of the
previous step is lifted through $\kappa_1$ to the upper right, and fresh
$\kappa_0(\bot)$ and $\bot$ are introduced.
...
\begin{tikzpicture}[scale=1.2]
  ...
  \path (2,0) node {$\bot$} [grow'=up, sibling distance=4.0em, level distance=1.9em]
    child { node {$\kappa_0(\bot)$} }
    child { node {$\kappa_1(\bot)$} }
  ;

  \path (2,-1) node {$F(1)$};

  \path (5,0) node {$\bot$} [grow'=up, sibling distance=4.0em, level distance=1.9em]
    child { node {$\kappa_0(\bot)$} }
    child {
      node { $\kappa_1(\bot)$ }
      child { node { $\kappa_1(\kappa_0(\bot))$ } }
      child { node { $\kappa_1(\kappa_1(\bot))$ } }
    }
  ;
...
\end{tikzpicture}
\end{center}
\caption{The first iterations of $1+X$ on $1$}

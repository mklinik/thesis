...
\todo{Scott-continuity is equivalent to topological continuity induced by the
Scott-topology.}
...
monotonicity as a first step, so that for a chain $P$, $\{\,
...
$f : A \arr A$ there exists a unique arrow $Y(f) : 1 \arr A$ such that $Y(f) = f
\circ Y(f)$.
...
This is the famous fixpoint theorem, which is often attributed to Tarski, but
actually due to Kleene.

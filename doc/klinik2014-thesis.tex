...
\lub S = \lub_{x \in S} x = \lub \{\, x \mid x \in S \,\}
\end{IEEEeqnarray*}
...
Note that if a function preserves lubs, it is automatically monotone,
\begin{IEEEeqnarray*}{rCl}
  x \below y \implies
  y = \lub \{\, x, y\, \} \implies
  f(y) = f(\lub \{\, x, y\, \}) = \lub \{\, f(x), f(y)\, \} \implies
  f(x) \below f(y)
\end{IEEEeqnarray*}

but we usually need to prove monotonicity as a first step for a proof of
continuity of a function $f$, as one step is often that for a chain $P$, $\{\,
f(x) \mid x \in P\,\}$ is again a chain. This motivates the inclusion of
...
The complete, pointed posets together with continuous functions form a category.
This category is called Cppo.
...
Two times left, at the traffic lights right.

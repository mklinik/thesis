...
steps. In particular we show that there exist mediating em\-bed\-ding projection
functions between the final coalgebras of the functors $1+X$ and $1+X+X$ in the
category of complete pointed partial orders. The construction is then
...
It is not hard to see that both constructions yield cppos, and that their
...
functor on the terminal object. (todo: and taking the limit) If we consider the functor $F(X) = 1 \oplus X$,
...
L_B(f) : A \oplus B \arr A' \oplus B'$.  Therefore $\oplus : \Cppo \times
\Cppo \arr \Cppo$ is a bifunctor. \qed
...
intuitively states that monotone functions preserve chains and lubs, while the
interaction between $S$ and $\below$ is required for $S$ to also reflect chains
and lubs.
...
A similar result holds for a chain in $B$ which eventually rises into $S$. The
constructor $\_$ is not relevant, so the proof proceeds just as in
\ref{lemChainInversion}:

\begin{lemma} \label{lemChainInversionB}
If $C$ is a chain in $B$ with $\lub C = S(b)$ for some $b \in B$, then $\lub C =
S(\lub C'')$ where $C'' = \set{x \mid S(x) \in C}$.  \qed
\end{lemma}
...
continuous functions.
\item \label{proofPiExists2} $\psi$ itself is continuous.

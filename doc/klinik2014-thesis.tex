...
$P \subseteq S$ has a least upper bound in $S$.
...
third implication is a bit more involved, especially in the uncountable case. We
give a proof for the countable case here, as it is illustrative, and refer to
the literature for the uncountable case.
...
$D$ be a cpo as in definition \ref{defCpoOmegaSequenceComplete}, let $S = \{\,x_0,
x_1, x_2, \ldots\,\} \subseteq D$ be countable and directed. Construct a sequence
of sets $P_n \subseteq S$ as follows,
\begin{IEEEeqnarray*}{rCl}
P_0 & = & \{\, x_0 \,\} \\
P_{n+1} & = & P_n \cup \{\, \lub (P_n \cup y_n), y_n \,\}
\end{IEEEeqnarray*}

where $y_n$ is the $x_i \in S\backslash P_n$ with the smallest possible $i$.
Every $P_n$ is finite, so taking lubs is justified. Consider the
$\omega$-sequence $Q$ = $\{\,\lub P_n \,\}_{n \in \mathbb{N}}$. For all $n$, we
have $P_n \subseteq P_{n+1}$ and thus $\lub P_n \below \lub P_{n+1}$, so $Q$ has
a lub $z$. Each $x \in S$ eventually occurs in some $P_n$, and we have $x \below
\lub P_n \below z$, so $z$ is an upper bound of $S$. Any upper bound $z'$ of $S$
is also an upper bound of $Q$, and therefore $z \below z'$, i.e. $z$ is the lub
of $S$.

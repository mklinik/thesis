...
prove that this is impossible, while looking for a dentotational semantics of
the untyped lambda calculus. Intuitively, this is possible because the
...
there are categories where initial objects behave differently, perhaps
unintuitively so. Important examples are categories where objects are algebraic
structures whose signatures require the existence of elements, or such existence
follows from the axioms. There, the initial objects are not empty sets. Think of
monoids, groups, topological spaces or, in fact, cppos. For an object $A$ in
such a category the equivalence $0 \cong 0 \product A$ usually doesn't hold.

\todo{check if these categories really have non-empty initial objects}
...
As concise as abstract proofs like this become, they aren't very
illustrative.  The following alternative proof of \ref{lem0xAisInitial} is a bit
longer, but more insightful. The idea is that the curried versions of all arrows
$0 \product A \arr B$ are the same, and so are the arrows themselves.

\begin{proof}
Again, we show that $0 \product A$ is initial. Let $B$ be an object. Then the
...
\matrix (m) [matrix of math nodes,row sep=3em,column sep=4em,minimum width=2em]
  ...
  \path[-stealth, dashed] (m-2-1) edge node [left] {$\overline{f}\!,\overline{g}$} (m-1-1);
  \path[-stealth] (m-1-2) edge node [above] {$\text{ev}$} (m-1-3)
    (m-2-2) edge node [left] {$\overline{f} \product \text{id}_A$} (m-1-2)
    (m-2-2) edge node [below right] {$f\!,g$} (m-1-3);
\end{tikzpicture}
\end{center}
\end{figure}

And we have: $\overline{f} = \overline{g}$, because $0$ is initial. Therefore
$\overline{f}\product \text{id}_A = \overline{g}\product \text{id}_A$, and
thus
...
$Y(\text{id}_0) : 1 \arr 0$.  The universal mapping properties of $0$ and $1$
give the isomorphism $0 \cong 1$.
\end{proof}

\begin{definition}
A category is \emph{trivial} iff it has, up to isomorphism, exactly one object
and one arrow, the identity.
\end{definition}
...
\begin{proof}
Let $A$ be an object. Then $A \cong 1 \product A \cong 0 \product A \cong 0
\cong 1$. The first equivalence holds always, the second because $- \product A$
is a functor, and the last two are lemmas \ref{lem0xAisInitial} and
\ref{lem0isomorphic1}.
\end{proof}
...
\begin{lemma}
In a CCC with coproducts, $2 = 1+1$ is a boolean algebra object.
\end{lemma}
...
\todo{Use separated sums, and explain what's wrong with coalesced sums. Hint:
least fixpoints of functors are trivial.}
...
are defined in the obvious way. They are continuous.
...
\begin{definition}
A \emph{bifunctor} is a binary functor. Formally, $F : \bbC \times \bbD \arr
...
: A \arr A'$ and $g : B \arr B'$, $L_{B'}(f) \circ R_A(g) = R_{A'}(g) \circ L_B(f) : A

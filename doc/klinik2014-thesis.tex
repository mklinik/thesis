...
both elements of and functions between the semantic domain $D$, we soon run into
the equation $D \cong D \arr D$ or slight variants thereof.
...
For both recursive domain equations and recursive function definitions, domain
theory offers tools to approach solutions.
...
A \emph{chain} $C$ in a poset is a non-empty linearly ordered subset, that is
...
\matrix (m) [matrix of math nodes,row sep=3em,column sep=6em,minimum width=2em]
  ...
  \path[-stealth]
    ...
    (m-1-1) edge node [above] {denote} (m-1-2)
    (m-2-1.east|-m-2-2) edge node [below] {denote} (m-2-2)
    (m-1-2) edge node [right] {evaluate} (m-2-2);
\end{tikzpicture}
\end{center}

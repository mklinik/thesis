...
type theory by using datatype of coinductive natural numbers. We study the
...
diverge.  As a result, not all recursive functions are representable as elements
of a function type in such type theories. In his PhD thesis, Venanzio Capretta
\cite{Capretta2002} shows how by using a datatype that allows infinite elements,
it is possible to represent recursive functions as elements of a function type
in his type theory.  He extends the formal system with the datatype of
\emph{coinductive natural numbers}, the greatest fixpoint of the functor $F(X) =
1 + X + X$.

In this thesis we loosely take up this idea and study the denotational semantics
of this datatype. Dana Scott, one of the pioneers of programming language
semantics, discovered that complete partial orders and continuous functions lend
themselves well for giving denotations to programs. Our study takes place in
this setting.

...
to show that $\varphi$ and $\psi$ are themselves arrows in Cppo.

\todo{between exponent objects $\varphi : A \Arr B \arr A \Arr B$ and $\psi : B
\Arr A \arr B \Arr A$}
...
$\varepsilon : A \arr B$ and $\pi : B \arr A$ using some categorical
craftsmanship: $[1 \arr A \Arr B] \cong [1\product A \arr B] \cong [A \arr B]$.
We have $\varepsilon = \varphi(\varepsilon)$ and $\pi = \psi(\pi)$, so we
calculate:
\begin{IEEEeqnarray*}{rCl}
\pi \circ \varepsilon & = & [\alpha^{-1} \circ F\pi, \pi] \circ \beta \circ
  \beta^{-1} \circ \kappa_0 \circ F\varepsilon \circ \alpha \\
& = & [\alpha^{-1} \circ F\pi, \pi] \circ \kappa_0 \circ F\varepsilon \circ
  \alpha \\
& = & \alpha^{-1} \circ F\pi \circ F\varepsilon \circ \alpha \\
& = & \alpha^{-1} \circ F(\pi \circ \varepsilon) \circ \alpha
\end{IEEEeqnarray*}
But this implies $\alpha \circ (\pi \circ \varepsilon)$ = $F(\pi \circ
\varepsilon) \circ \alpha$, so by finality of $\alpha$ just as in the proof of
\ref{propEpsilonPiExist}, $\pi \circ \varepsilon = \text{id}_A$.

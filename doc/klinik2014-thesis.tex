...
If $a = S(a')$ then by transfinite induction on the length of a,

\todo{so far everything should generalize to the functors and the final
coalgebras.  What about this last step?}
\begin{IEEEeqnarray*}{c}
\pi(\varepsilon(S(a'))) = \pi(S(\varepsilon(a'))) = S(\pi(\varepsilon(a'))) =
S(a')
\end{IEEEeqnarray*}
